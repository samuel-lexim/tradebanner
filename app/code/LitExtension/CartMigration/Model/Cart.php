...
//         curl_setopt( $ch, CURLOPT_USERAGENT, $_SERVER['HTTP_USER_AGENT']);
    ...
    public function generateCategoryUrlKey($name) {
        ...
        $cur_url_key = $url_def;
        while ($this->_checkUrlKeyExisted($cur_url_key, 'category')) {
            $i++;
            $cur_url_key = $url_def . '-' . $i;
        }
        return $cur_url_key;
    ...
    public function setProductRelation($src_product, $desc_product, $link_type = 1, $both = false) {
        ...
        foreach ($relate_products as $product_src_id) {
            ...
            if ($flag) {
                $this->insertTable("catalog_product_link", array('product_id' => $product_mage_id, 'linked_product_id' => $partner_id, 'link_type_id' => $link_type));
            }
            if ($both || !$flag) {
                $this->insertTable("catalog_product_link", array('product_id' => $partner_id, 'linked_product_id' => $product_mage_id, 'link_type_id' => $link_type));
            }
        }
    }

    public function generateUrlKeyFromUrlKey($url_def, $type = 'product') {
        $i = 0;
        $cur_url_key = $url_def;
        while ($this->_checkUrlKeyExisted($cur_url_key, $type)) {
            $i++;
            $cur_url_key = $url_def . '-' . $i;
        }
        return $cur_url_key;
    ...
    protected function _checkUrlKeyExisted($url_key, $type = 'product') {
        $map = array(
            'product' => array(
        ...
        $table_prd = $this->getTableName($map[$type]['table']);
        $table_eav = $this->getTableName('eav_attribute');
        $table_type = $this->getTableName('eav_entity_type');
        $query = "SELECT a.* FROM ".$table_prd." AS a, ".$table_eav." AS b, ".$table_type." AS c
                    WHERE b.attribute_code = 'url_key'
                    AND b.attribute_id = a.attribute_id
                    AND b.entity_type_id = c.entity_type_id
                    AND c.entity_type_code = '" . $map[$type]['entity_type_code'] . "'
                    AND a.value = '" . $url_key . "'";
        try{
            $result = $this->_db->fetchAll($query);
